...
class APICollectionApi(object):
    ...
    def apis_post(self, body, content_type, **kwargs):
        """
        Create a new API
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.apis_post(body, content_type, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param APIObject body: API object that needs to be added  (required)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.apis_post_with_http_info(body, content_type, **kwargs)
        else:
            (data) = self.apis_post_with_http_info(body, content_type, **kwargs)
            return data

    def apis_post_with_http_info(self, body, content_type, **kwargs):
        """
        Create a new API
        This operation can be used to create a new API specifying the details of the API in the payload. The new API will be in `CREATED` state.  There is a special capability for a user who has `APIM Admin` permission such that he can create APIs on behalf of other users. For that he can to specify `\"provider\" : \"some_other_user\"` in the payload so that the API's creator will be shown as `some_other_user` in the UI. 
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.apis_post_with_http_info(body, content_type, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method apis_post" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'body' is set
        if ('body' not in params) or (params['body'] is None):
            raise ValueError("Missing the required parameter `body` when calling `apis_post`")
        # verify the required parameter 'content_type' is set
        if ('content_type' not in params) or (params['content_type'] is None):
            raise ValueError("Missing the required parameter `content_type` when calling `apis_post`")
        ...
        return self.api_client.call_api('/apis', 'POST',
                                        path_params,
                                        query_params,
                                        header_params,
                                        body=body_params,
